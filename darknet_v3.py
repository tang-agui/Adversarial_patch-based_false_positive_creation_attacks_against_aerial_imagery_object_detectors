...
if __name__ == "__main__":
    ...
    x = torch.randn(4, 3, 608, 608)  # 这个给出的输入大小会覆盖配置文件中的参数
    x = x.to(device)
    ...
    net = Darknet(cfgfile)
    print("net : ", net)
    net.load_darknet_weights(weightfile)
    # net = net.cuda()
    net = net.eval().to(device)
    outputs = net(x)
    # print("eval output size : ", outputs.size())
    for output in outputs:
        # print(output[:,:,4].size())
        # print("confidence : ", output[:,:,4].max())
        print("output size of Darknet ", output.size())
    # img = torch.autograd.Variable(x)
